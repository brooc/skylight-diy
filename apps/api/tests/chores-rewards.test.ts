...
describe("chores and rewards", () => {
  ...
  it("schedules, edits, archives, restores, spends, resets, and preserves point history", async () => {
    ...
    const archived = await app.inject({ method: "DELETE", url: `/api/chores/${task.id}` });
    ...
    expect((await app.inject({ method: "POST", url: `/api/chores/${task.id}/restore` })).statusCode).toBe(200);

    const spent = await app.inject({ method: "POST", url: `/api/rewards/${kiddo.id}/reduce`, payload: { amount: 2, reason: "Screen time" } });
    expect(spent.statusCode).toBe(201);
    expect(spent.json().balance).toBe(3);
    const overspend = await app.inject({ method: "POST", url: `/api/rewards/${kiddo.id}/reduce`, payload: { amount: 4, reason: "Too much" } });
    expect(overspend.statusCode).toBe(409);
    expect((await app.inject({ method: "POST", url: `/api/rewards/${kiddo.id}/reset` })).statusCode).toBe(200);
  ...
  it("requires a family member when anyone completes a one-time task", async () => {
    ...
    const household = await app.inject({ method: "GET", url: "/api/household/current" });
    const kiddo = household.json().people.find((person: { displayName: string }) => person.displayName === "Kiddo");
    const created = await app.inject({ method: "POST", url: "/api/chores", payload: { title: "Return book", points: 2, frequency: "once", dueDate: "2026-07-19" } });
    const task = created.json().chore;
